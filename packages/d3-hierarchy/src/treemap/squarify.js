...
export function squarifyRatio(ratio, parent, x0, y0, x1, y1) {
  ...
  while (i0 < n) {
    ...
    // 第一轮先根据parent节点的空间区域，计算宽高值
    dx = x1 - x0, dy = y1 - y0;

    // nodes表示parent的孩子节点，不断遍历直到出现第一个value不为0的孩子节点
    do sumValue = nodes[i1++].value; 
    while (!sumValue && i1 < n);

    minValue = maxValue = sumValue;

    // Math.max(dydx,dx/dy)计算宽长比或者长宽比。下面
    ...
    // 如果当前矩形宽大于高，则按照垂直方向切割矩形的宽度；反之水平方向切割矩形的高度
    rows.push(row = {
      value: sumValue, dice: dx < dy, children: nodes.slice(i0, i1)});
    if (row.dice) 
